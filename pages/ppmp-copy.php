...
<script>
  ...
  // ADD PROCESS
    ...
    document.getElementById("addBtn").addEventListener("click", function() {
      ...
      const itemNameSelect = document.getElementById("item_name_id");
      const itemNameId = itemNameSelect.value;
      const itemName = itemNameSelect.options[itemNameSelect.selectedIndex]?.text || "";
      ...
      if (!categoryId || !subCategoryId || !itemNameId || !itemDescription || !specification || !quantity || !unitOfMeasure || !unitCost || !quarterNeeded || !justification) {
      ...
      row.setAttribute("data-item-name-id", itemNameId); 
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success && response.data.length > 0) {
            ...
            $itemSelect.append('<option value="" disabled selected>No item names available</option>');
          }
          $itemSelect.val(null).trigger('change.select2'); 
    ...
    $('#SaveForm').submit(function (e) {
      ...
      $('#ppmpTable tbody tr').each(function() {
        ...
        items.push({
          ...
          item_name_id: row.data('item-name-id'),
